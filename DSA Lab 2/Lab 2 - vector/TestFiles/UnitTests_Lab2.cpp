...
FailResult UnitTests_Lab2::Fail_PalindromeNumber_IsNotPalindromeNumber() {
	...
	for (auto iter = nonPalindromeVec.begin(); iter != nonPalindromeVec.end(); ++iter) {
		if ((bool)IsPalindromeNumber(*iter) == 1) {
			msg += std::to_string(*iter) + " is registering as a palindrome number (do all control paths return a value?)";
			success = false;
			break;
		}
	}
